/* NetId(s): djg17, sw744. Time spent: 10 hours, 30 minutes.
...
public class BugTree {
	...
	public BugTree add(Human p, Human c) throws IllegalArgumentException {
		...
		if (getTree(p) == null ) throw new IllegalArgumentException();
		if (contains(c)) {throw new IllegalArgumentException();}
		BugTree a= getTree(p);

		a.children.add(new BugTree(c));
		return getTree(c);
	}
	...
	public int depthOf(Human p) {
		...
		//is it ok that we use "get tree" instead of returning -1 and doing an if statement to catch it?

		if (p == root) return 0;
		//For-Each loop to check each children in the object
		for (BugTree t : children) {
			//if child t of the root contains p
			if (t.getTree(p) != null) { 
				return 1 + t.depthOf(p);
			}
		}
		return -1;	
	...
	public List<Human> bugRouteTo(Human c) {
		...
		LinkedList<Human> a = new LinkedList<Human>();
		if(c==root){
			a.addFirst(c);
			return a;
		}
		for (BugTree child: children){
			LinkedList<Human> b = (LinkedList<Human>) child.bugRouteTo(c);
			if(b!=null){
				b.addFirst(root);
				return b;
			}
	...
	public Human sharedAncestorOf(Human child1, Human child2) {
		...
		//we don't need contains. Less efficient apparently
		//specs say they have to be in the same tree, so we dont need this
		//Also, bugRoute will return null, if child1 and child2 are not in the same tree or
		//shareAncestor
...
/*		if(!contains(child1) || !contains(child2)){
		...
		List<Human> a = bugRouteTo(child1);
		List<Human> b = bugRouteTo(child2);
		int size = Math.min(a.size(), b.size());
		//start i at 1, since you know the first element of each list is the same since you verified they 
	...
	public boolean equals(Object ob) {
		...
		boolean checkroot = (root==((BugTree) ob).getRoot());
		//if(checkroot){return true;}
		int size1 = this.size();
		int size2 = ((BugTree) ob).size();

		boolean matchChildren = size1==size2;

		if (!matchChildren) return false;
		//write a for loop to check each child of bt as well
		//boolean check = true;
		for(BugTree bugchild: ((BugTree) ob).copyOfChildren()){

			if(!help(bugchild, children)){
				return false;
			}
		}
		//I changed this!!!!!!!!!!!!
	...
	/** Return true iff bt equals some member of b2 */
	private boolean help(BugTree bt, Set<BugTree> b2){

		for(BugTree child: b2){
			if(bt.equals(child)){
				return true;
			}
		}
